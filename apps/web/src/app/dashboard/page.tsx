...
const onboardingGoalLabels: Record<OnboardingGoal, string> = {
  LEARN: "Learn practical skills",
  PROVE: "Build proof of work",
...
export default function DashboardPage() {
  ...
  const [onboardingState, setOnboardingState] =
    useState<MemberOnboardingState | null>(null);
  ...
  function getOnboardingResumePath() {
    if (!onboardingState) {
      return "/onboarding";
    }

    const goal = encodeURIComponent(onboardingState.primaryGoal);

    if (!hasProfile) {
      return `/onboarding/profile?goal=${goal}`;
    }

    if (skills.length < 2) {
      return `/onboarding/skills?goal=${goal}`;
    }

    if (interests.length < 2) {
      return `/onboarding/interests?goal=${goal}`;
    }
  ...
  function isProofReviewed(proof: ProofSubmission) {
    const reviewStatus = proof.reviewStatus?.toUpperCase();
  ...
  function getReviewedProofsCount() {
    return proofs.filter(isProofReviewed).length;
  }

  function getAverageVerificationScore() {
    const reviewedProofs = proofs.filter(isProofReviewed);

    if (reviewedProofs.length === 0) {
      return 0;
    }

    const total = reviewedProofs.reduce((sum, proof) => {
      return sum + (proof.verificationScore || 0);
    }, 0);

    return Math.round(total / reviewedProofs.length);
  }

  function getProofState(proof: ProofSubmission) {
    if (isProofReviewed(proof)) {
      return proof.reviewStatus || proof.status || "SUBMITTED";
    }
    ...
    async function loadDashboardData() {
      try {
        const [
          ...
          getMemberInterests(authenticatedUserId).catch(() => []),
          getMyOnboardingState().catch(() => null),
        ]);
        ...
        setHasProfile(Boolean(loadedProfile));
  ...
  async function handleCustomSkillSubmit(event: SubmitEvent<HTMLFormElement>) {
  ...
  async function handleCreateProof(event: SubmitEvent<HTMLFormElement>) {
    event.preventDefault();
              ...
              <section className="space-y-5">
                ...
                {isTodaySection && (
                  <>
                    {!onboardingState?.onboardingCompleted && (
                      <div className="border border-[#B9DDF5] bg-[#EAF3FF] p-6 shadow-sm">
                        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#0890E0]">
                              Complete your starting setup
                            </p>

                            <h2 className="mt-2 text-xl font-black">
                              {getOnboardingCompletedSteps()} of 4 steps
                              complete
                            </h2>

                            <p className="mt-2 max-w-2xl text-sm font-bold leading-6 text-[#102848]/60">
                              {onboardingState
                                ? `Your current direction is “${
                                    onboardingGoalLabels[
                                      onboardingState.primaryGoal
                                    ]
                                  }”. Finish the remaining setup so GUMMI can guide your journey properly.`
                                : "Choose what you want GUMMI to help you achieve first, then complete your starting profile."}
                            </p>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              router.push(getOnboardingResumePath())
                            }
                            className="shrink-0 rounded-xl bg-[#0890E0] px-5 py-3 text-sm font-black text-white"
                          >
                            Continue setup
                          </button>
                        </div>
                ...
                {isSkillsSection && (
                  <div className="border border-[#DCE7F2] bg-white/95 p-6 shadow-sm">
                    ...
                    <p className="mt-2 max-w-2xl text-sm font-bold leading-7 text-[#102848]/60">
                      Add the skills you are practicing. Later, GUMMI will
                      connect these skills to challenges, projects, proof of
                ...
                {isProfileSection && (
                  ...
                  >
                    ...
                    <p className="mt-2 max-w-2xl text-sm font-bold leading-7 text-[#102848]/60">
                      Your profile should help members, experts, and
                      organizations understand your direction, your story, and
